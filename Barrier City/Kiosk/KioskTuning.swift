...
enum KioskTuning {
    ...
    ///
    /// 값의 유래(KioskScreenView 지오메트리에서 역산 — 아래 넷 중 하나라도 바뀌면 함께 고쳐야 한다):
    ///   패널 높이 1090pt, 상단 존 높이 200pt, 환산 1m ≈ 1360pt, screenCenterY 1.25m
    ///   → 패널 높이 1090/1360 = 0.801m, 상단 = 1.25 + 0.801/2 = 1.651m
    ///   → 상단 존 하단 = 1.651 − 200/1360 = 1.504m ≒ 1.50
    /// 이보다 낮게 잡으면 눈에 보이는 상단 존보다 아래에 있는 손이 '닿았다'로 판정되어
    /// 장벽 자체가 무력화된다(리치 판정은 관대한 쪽으로 틀리면 안 된다).
    static let upperZoneMinY: Float = 1.50
    /// 리치 판정 여유(m). 존 경계보다 이만큼 아래까지는 닿은 것으로 인정.
    /// 판정에서 '빼는' 값이라 키울수록 판정이 관대해진다 — 장벽을 지키려면 작게 유지할 것.
