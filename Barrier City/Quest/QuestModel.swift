...
enum QuestTuning {
    ...
    static let centerVerticalOffset: Float = -0.08

    // MARK: 튜토리얼 전용 배치
    //
    // 텍스트 카드는 손 닿는 거리에 낮게 두고 사용자를 향해 눕힌다(책상 위 패널).
    // 영상은 멀리 눈높이에 세워 TV처럼 본다.
    /// 텍스트 카드까지의 수평 거리(m).
    static let cardDistance: Float = 0.70
    /// 텍스트 카드의 눈높이 대비 높이(m).
    static let cardVerticalOffset: Float = -0.70
    /// 시선각 대비 실제로 적용할 눕힘 비율. 1이면 정확히 정면으로 마주 본다.
    /// 1보다 작으면 그만큼 덜 눕혀 조금 더 세워진다.
    static let cardPitchRatio: Float = 0.8
    /// 카드가 사용자를 향해 눕는 최대 각도(rad). 실제로는 시선각을 따라간다.
    static let cardMaxPitch: Float = 70 * .pi / 180
    /// 영상 패널까지의 수평 거리(m).
    static let videoDistance: Float = 2.2
    /// 영상 패널의 눈높이 대비 높이(m).
