        ...
        pendingFunctionCalls.append(FunctionCall(
            ...
            followUpInstructions: isFirstAttempt
                ? """
                  ...
                  대안을 제시하거나, 이 규칙을 나중에 다시 언급하지 마라. 방문자가 이미 같은 호흡에
                  키오스크를 못 쓰는 이유를 설명했더라도, 이번 턴은 여전히 순수하게 거절/리다이렉트
                  대사여야 한다 — "알겠어요", "받아둘게요", "그거 하나만요" 같이 승낙하는 것처럼
                  들리는 표현은 이번 턴에 절대 쓰지 마라. 실제 승낙은 place_mission_order가 성공한
                  다음 턴에만 한다.
                  """
                : """
